...
//
// Package:     Muons
// Class  :     FWRPCRecRecHitProxyBuilder
//
// Implementation:
...
class FWRPCRecHitProxyBuilder : public REveDataSimpleProxyBuilderTemplate<RPCRecHit>
{
public:
   REGISTER_FWPB_METHODS();
   virtual bool HaveSingleProduct() const {return false;}

   using REveDataSimpleProxyBuilderTemplate<RPCRecHit>::BuildItemViewType; 
   virtual void BuildItemViewType(const RPCRecHit& iData, int idx,  REveElement* oItemHolder, const std::string& type , const REveViewContext*) override
    {
      ...
      if (!geom->contains(rawid)) {
         fwLog(fwlog::kError) << "failed to get geometry of RPC roll with detid: " << rawid << std::endl;
         return;
      }
...
REGISTER_FW2PROXYBUILDER(FWRPCRecHitProxyBuilder, RPCRecHit, "RPC RecHits");
