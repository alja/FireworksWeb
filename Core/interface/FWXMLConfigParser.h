...
public:
  FWXMLConfigParser(std::istream &f) : SimpleSAXParser(f), m_state(IN_BEGIN_DOCUMENT), m_first(nullptr) {}
  ...
  /** The parsed configuration. Notice that the parser owns it and destroys
       it when destroyed.
     */
  FWConfiguration *config(void) { return m_first.get(); }
  ...
  void debug_config_state_machine(const char *where, const std::string &tag, int state) {
    ...
    static char *debug_states[] = {
        "IN_BEGIN_DOCUMENT", "IN_PUSHED_CONFIG", "IN_POPPED_CONFIG", "IN_BEGIN_STRING", "IN_STORED_STRING"};
