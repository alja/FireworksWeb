...
REveGeoShape* FWGeometry::getEveShape(unsigned int id) const {
  IdToInfoItr it = FWGeometry::find(id);
  if (it == m_idToInfo.end()) {
    fwLog(fwlog::kWarning) << "no reco geoemtry found for id " << id << std::endl;
    return nullptr;
    ...
    double array[16] = {info.matrix[0],
                        ...
                        1.};
    REveGeoManagerHolder gmgr(REveGeoShape::GetGeoManager());
    REveGeoShape* shape = new REveGeoShape(TString::Format("RecoGeom Id=%u", id).Data());
...
REveGeoShape* FWGeometry::getHGCSiliconEveShape(unsigned int id) const {
#if 0
...
REveGeoShape* FWGeometry::getHGCScintillatorEveShape(unsigned int id) const {
  IdToInfoItr it = FWGeometry::find(id);
  if (it == m_idToInfo.end()) {
    fwLog(fwlog::kWarning) << "no reco geometry found for id " << id << std::endl;
    return nullptr;
