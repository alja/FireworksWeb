...
#include "FireworksWeb/Core/src/fwPaletteClassic.icc"
#include "FireworksWeb/Core/src/fwPaletteExtra.h"
...
void FWColorManager::setDefaultGeomColors() {
  m_geomColor[kFWPixelBarrelColorIndex] = kGray;
  m_geomColor[kFWPixelEndcapColorIndex] = kGray + 1;

  m_geomColor[kFWTrackerBarrelColorIndex] = kGreen+3;
  ...
  m_geomColor[kFWMtdBarrelColorIndex] = kMagenta;
  m_geomColor[kFWMtdEndcapColorIndex] = kMagenta+2;
/*
// temprary inactive
  switch (m_paletteId) {
    case (kArctic):
...
void FWColorManager::initialize() {
  ...
  return; // AMT  cleint does not know about this colors
  ...
  REveUtil::SetColorBrightness(0, kFALSE);
}
